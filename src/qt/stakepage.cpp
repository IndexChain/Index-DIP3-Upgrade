...
{
    ui->setupUi(this);
    ui->checkStake->setEnabled(GetBoolArg("-staking", true));
...
void StakePage::setClientModel(ClientModel *_clientModel)
{
    ...
    if (_clientModel) {
        ...
        int height = _clientModel->getNumBlocks();
        ui->labelHeight->setText(QString::number(height));
        m_subsidy = _clientModel->getBlockSubsidy(height);
        m_networkWeight = _clientModel->getNetworkStakeWeight();
        m_expectedAnnualROI = _clientModel->getEstimatedAnnualROI();
        updateNetworkWeight();
        updateAnnualROI();
    }
}
...
void StakePage::setWalletModel(WalletModel *model)
{
    ...
    if(model && model->getOptionsModel())
    {
        ...
        transactionView->chooseType(9);
        ui->checkStake->setChecked(model->getEnabledStaking());
...
void StakePage::setBalance()
{
    int unit = walletModel->getOptionsModel()->getDisplayUnit();
    ui->labelAssets->setText(BitcoinUnits::formatWithUnit(unit, walletModel->getBalance(), false, BitcoinUnits::separatorAlways));
    ui->labelStake->setText(BitcoinUnits::formatWithUnit(unit, walletModel->getImmatureBalance(), false, BitcoinUnits::separatorAlways));
}
...
void StakePage::on_checkStake_clicked(bool checked)
{
    ...
    this->walletModel->setEnabledStaking(checked);
...
void StakePage::numBlocksChanged(int count, const QDateTime &, double, bool headers)
{
    if(!headers && clientModel && walletModel)
    {
        ...
        m_subsidy = clientModel->getBlockSubsidy(count);
        m_networkWeight = clientModel->getNetworkStakeWeight();
        m_expectedAnnualROI = clientModel->getEstimatedAnnualROI();
        updateSubsidy();
        updateNetworkWeight();
        updateAnnualROI();
    }
...
void StakePage::updateEncryptionStatus()
{
    ...
    case WalletModel::Unlocked:
        if(walletModel->getEnabledStaking())
        {
