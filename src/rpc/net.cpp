...
UniValue getpeerinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw runtime_error(
            ...
            "    \"verified_proregtx_hash\": h, (hex) Only present when the peer is a indexnode and succesfully\n"
            "                               autheticated via MNAUTH. In this case, this field contains the\n"
            "                               protx hash of the indexnode\n"
...
UniValue getaddednodeinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:8333\",  (string) The Index server IP and port we're connected to\n"
