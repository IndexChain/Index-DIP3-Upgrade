...
CZnode::CZnode() :
        ...
        nBlockLastPaid(0),
        nProtocolVersion(LEGACY_INDEXNODES_PROTOCOL_VERSION),
        nPoSeBanScore(0),
        nPoSeBanHeight(0),
        fAllowMixingTx(true),
        fUnitTest(false) {}
...
bool CZnode::UpdateFromNewBroadcast(CZnodeBroadcast &mnb) {
    ...
    if (fMasternodeMode && pubKeyZnode == activeZnode.pubKeyZnode) {
        ...
        if (nProtocolVersion == LEGACY_INDEXNODES_PROTOCOL_VERSION) {
...
void CZnode::Check(bool fForce) {
    ...
    LogPrint("indexnode", "CZnode::Check -- Znode %s is in %s state\n", vin.prevout.ToStringShort(), GetStateString());
    ...
    if (!fUnitTest) {
        ...
        if (!pcoinsTip->GetCoin(vin.prevout, coin) || coin.out.IsNull() || coin.IsSpent()) {
            nActiveState = INDEXNODE_OUTPOINT_SPENT;
            LogPrint("indexnode", "CZnode::Check -- Failed to find Znode UTXO, indexnode=%s\n", vin.prevout.ToStringShort());
            return;
        }
    ...
    if (IsPoSeBanned()) {
        ...
        nActiveState = INDEXNODE_POSE_BAN;
...
/*    bool fRequireUpdate = nProtocolVersion < znpayments.GetMinZnodePaymentsProto() ||
    ...
    bool fRequireUpdate = nProtocolVersion < znpayments.GetMinZnodePaymentsProto() ||
                          // or it's our own node and we just updated it to the new protocol but we are still waiting for activation ...
                          (fOurZnode && (nProtocolVersion < MIN_INDEXNODE_PAYMENT_PROTO_VERSION_1 || nProtocolVersion > MIN_INDEXNODE_PAYMENT_PROTO_VERSION_2));

    if (fRequireUpdate) {
        nActiveState = INDEXNODE_UPDATE_REQUIRED;
        if (nActiveStatePrev != nActiveState) {
            LogPrint("indexnode", "CZnode::Check -- Znode %s is in %s state now\n", vin.prevout.ToStringShort(), GetStateString());
        }
        return;
    }

    // keep old indexnodes on start, give them a chance to receive updates...
    bool fWaitForPing = !indexnodeSync.IsZnodeListSynced() && !IsPingedWithin(INDEXNODE_MIN_MNP_SECONDS);

    if (fWaitForPing && !fOurZnode) {
        // ...but if it was already expired before the initial check - return right away
        if (IsExpired() || IsWatchdogExpired() || IsNewStartRequired()) {
            LogPrint("indexnode", "CZnode::Check -- Znode %s is in %s state, waiting for ping\n", vin.prevout.ToStringShort(), GetStateString());
            return;
        }
    }
    ...
    if (!fWaitForPing || fOurZnode) {
        ...
        if (!IsPingedWithin(INDEXNODE_NEW_START_REQUIRED_SECONDS)) {
            nActiveState = INDEXNODE_NEW_START_REQUIRED;
            if (nActiveStatePrev != nActiveState) {
                LogPrint("indexnode", "CZnode::Check -- Znode %s is in %s state now\n", vin.prevout.ToStringShort(), GetStateString());
            }
            return;
...
//        LogPrint("indexnode", "CZnode::Check -- outpoint=%s, nTimeLastWatchdogVote=%d, GetTime()=%d, fWatchdogExpired=%d\n",
//                vin.prevout.ToStringShort(), nTimeLastWatchdogVote, GetTime(), fWatchdogExpired);
        ...
        if (!IsPingedWithin(INDEXNODE_EXPIRATION_SECONDS)) {
            nActiveState = INDEXNODE_EXPIRED;
            if (nActiveStatePrev != nActiveState) {
                LogPrint("indexnode", "CZnode::Check -- Znode %s is in %s state now\n", vin.prevout.ToStringShort(), GetStateString());
            }
            return;
    ...
    if (Params().NetworkIDString() != CBaseChainParams::REGTEST && lastPing.sigTime - sigTime < INDEXNODE_MIN_MNP_SECONDS) {
        nActiveState = INDEXNODE_PRE_ENABLED;
        if (nActiveStatePrev != nActiveState) {
            LogPrint("indexnode", "CZnode::Check -- Znode %s is in %s state now\n", vin.prevout.ToStringShort(), GetStateString());
        }
        return;
    ...
    nActiveState = INDEXNODE_ENABLED; // OK
    if (nActiveStatePrev != nActiveState) {
        LogPrint("indexnode", "CZnode::Check -- Znode %s is in %s state now\n", vin.prevout.ToStringShort(), GetStateString());
    }
}
...
indexnode_info_t CZnode::GetInfo() {
...
void CZnode::UpdateLastPaid(const CBlockIndex *pindex, int nMaxBlocksToScanBack) {
    ...
    LogPrint("indexnode", "CZnode::UpdateLastPaidBlock -- searching for block with payment to %s\n", vin.prevout.ToStringShort());
...
//        LogPrintf("znpayments.mapZnodeBlocks[BlockReading->nHeight].HasPayeeWithVotes(mnpayee, 2)=%s\n", znpayments.mapZnodeBlocks[BlockReading->nHeight].HasPayeeWithVotes(mnpayee, 2));
        if (znpayments.mapZnodeBlocks.count(BlockReading->nHeight) &&
            ...
            CAmount nZnodePayment = GetZnodePayment(params,pindex->nHeight);

            BOOST_FOREACH(CTxOut txout, block.vtx[0]->vout)
            if (mnpayee == txout.scriptPubKey && nZnodePayment == txout.nValue) {
                nBlockLastPaid = BlockReading->nHeight;
                nTimeLastPaid = BlockReading->nTime;
                LogPrint("indexnode", "CZnode::UpdateLastPaidBlock -- searching for block with payment to %s -- found new %d\n", vin.prevout.ToStringShort(), nBlockLastPaid);
                return;
            }
        }
    ...
    // Last payment for this indexnode wasn't found in latest znpayments blocks
    // or it was found in znpayments blocks but wasn't found in the blockchain.
    // LogPrint("indexnode", "CZnode::UpdateLastPaidBlock -- searching for block with payment to %s -- keeping old %d\n", vin.prevout.ToStringShort(), nBlockLastPaid);
}
...
bool CZnodeBroadcast::Create(std::string strService, std::string strKeyZnode, std::string strTxHash, std::string strOutputIndex, std::string &strErrorRet, CZnodeBroadcast &mnbRet, bool fOffline) {
    ...
    if (!darkSendSigner.GetKeysFromSecret(strKeyZnode, keyZnodeNew, pubKeyZnodeNew)) {
        strErrorRet = strprintf("Invalid indexnode key %s", strKeyZnode);
        LogPrintf("CZnodeBroadcast::Create -- %s\n", strErrorRet);
        return false;
    }
...
bool CZnodeBroadcast::Create(CTxIn txin, CService service, CKey keyCollateralAddressNew, CPubKey pubKeyCollateralAddressNew, CKey keyZnodeNew, CPubKey pubKeyZnodeNew, std::string &strErrorRet, CZnodeBroadcast &mnbRet) {
    ...
    LogPrint("indexnode", "CZnodeBroadcast::Create -- pubKeyCollateralAddressNew = %s, pubKeyZnodeNew.GetID() = %s\n",
    ...
    mnbRet = CZnodeBroadcast(service, txin, pubKeyCollateralAddressNew, pubKeyZnodeNew, LEGACY_INDEXNODES_PROTOCOL_VERSION);
...
bool CZnodeBroadcast::SimpleCheck(int &nDos) {
    ...
    if (!IsValidNetAddr()) {
        LogPrintf("CZnodeBroadcast::SimpleCheck -- Invalid addr, rejected: indexnode=%s  addr=%s\n",
    ...
    if (sigTime > GetAdjustedTime() + 60 * 60) {
        LogPrintf("CZnodeBroadcast::SimpleCheck -- Signature rejected, too far into the future: indexnode=%s\n", vin.prevout.ToStringShort());
        nDos = 1;
        return false;
    }
    ...
    if (nProtocolVersion < znpayments.GetMinZnodePaymentsProto()) {
        LogPrintf("CZnodeBroadcast::SimpleCheck -- ignoring outdated Znode: indexnode=%s  nProtocolVersion=%d\n", vin.prevout.ToStringShort(), nProtocolVersion);
        return false;
    }
...
bool CZnodeBroadcast::Update(CZnode *pmn, int &nDos) {
    ...
    if (pmn->IsPoSeBanned()) {
        LogPrintf("CZnodeBroadcast::Update -- Banned by PoSe, indexnode=%s\n", vin.prevout.ToStringShort());
        return false;
    }
    ...
    if (!CheckSignature(nDos)) {
        LogPrintf("CZnodeBroadcast::Update -- CheckSignature() failed, indexnode=%s\n", vin.prevout.ToStringShort());
        return false;
    }

    // if ther was no indexnode broadcast recently or if it matches our Znode privkey...
    if (!pmn->IsBroadcastedWithin(INDEXNODE_MIN_MNB_SECONDS) || (fMasternodeMode && pubKeyZnode == activeZnode.pubKeyZnode)) {
...
bool CZnodeBroadcast::CheckOutpoint(int &nDos) {
    // we are a indexnode with the same vin (i.e. already activated) and this mnb is ours (matches our Znode privkey)
    ...
    if (!CheckSignature(nDos)) {
        LogPrintf("CZnodeBroadcast::CheckOutpoint -- CheckSignature() failed, indexnode=%s\n", vin.prevout.ToStringShort());
        return false;
    }

    {
        ...
        if (!lockMain) {
            ...
            LogPrint("indexnode", "CZnodeBroadcast::CheckOutpoint -- Failed to aquire lock, addr=%s", addr.ToString());
        ...
        Coin coin;
        if (!pcoinsTip->GetCoin(vin.prevout, coin) || coin.out.IsNull() || coin.IsSpent()) {
            LogPrint("indexnode", "CZnodeBroadcast::CheckOutpoint -- Failed to find Znode UTXO, indexnode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
        if (coin.out.nValue != INDEXNODE_COIN_REQUIRED * COIN) {
            LogPrint("indexnode", "CZnodeBroadcast::CheckOutpoint -- Znode UTXO should have 1000 XZC, indexnode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
        if (chainActive.Height() - coin.nHeight + 1 < Params().GetConsensus().nZnodeMinimumConfirmations) {
            LogPrintf("CZnodeBroadcast::CheckOutpoint -- Znode UTXO must have at least %d confirmations, indexnode=%s\n",
    ...
    LogPrint("indexnode", "CZnodeBroadcast::CheckOutpoint -- Znode UTXO verified\n");
...
bool CZnodeBroadcast::CheckSignature(int &nDos) {
    ...
    LogPrint("indexnode", "CZnodeBroadcast::CheckSignature -- strMessage: %s  pubKeyCollateralAddress address: %s  sig: %s\n", strMessage, CBitcoinAddress(pubKeyCollateralAddress.GetID()).ToString(), EncodeBase64(&vchSig[0], vchSig.size()));
...
bool CZnodePing::CheckSignature(CPubKey &pubKeyZnode, int &nDos) {
    ...
    if (!darkSendSigner.VerifyMessage(pubKeyZnode, vchSig, strMessage, strError)) {
        LogPrintf("CZnodePing::CheckSignature -- Got bad Znode ping signature, indexnode=%s, error: %s\n", vin.prevout.ToStringShort(), strError);
        nDos = 33;
        return false;
    }
    return true;
...
bool CZnodePing::SimpleCheck(int &nDos) {
    ...
    if (sigTime > GetAdjustedTime() + 60 * 60) {
        LogPrintf("CZnodePing::SimpleCheck -- Signature rejected, too far into the future, indexnode=%s\n", vin.prevout.ToStringShort());
        nDos = 1;
        return false;
    }
...
//        LOCK(cs_main);
        ...
        if (mi == mapBlockIndex.end()) {
            LogPrint("indexnode", "CZnodePing::SimpleCheck -- Znode ping is invalid, unknown block hash: indexnode=%s blockHash=%s\n", vin.prevout.ToStringShort(), blockHash.ToString());
            // maybe we stuck or forked so we shouldn't ban this node, just fail to accept this ping
            // TODO: or should we also request this block?
            return false;
        }
    }
    LogPrint("indexnode", "CZnodePing::SimpleCheck -- Znode ping verified: indexnode=%s  blockHash=%s  sigTime=%d\n", vin.prevout.ToStringShort(), blockHash.ToString(), sigTime);
    return true;
}
...
bool CZnodePing::CheckAndUpdate(CZnode *pmn, bool fFromNewBroadcast, int &nDos) {
    ...
    if (pmn == NULL) {
        LogPrint("indexnode", "CZnodePing::CheckAndUpdate -- Couldn't find Znode entry, indexnode=%s\n", vin.prevout.ToStringShort());
        return false;
    }

    if (!fFromNewBroadcast) {
        if (pmn->IsUpdateRequired()) {
            LogPrint("indexnode", "CZnodePing::CheckAndUpdate -- indexnode protocol is outdated, indexnode=%s\n", vin.prevout.ToStringShort());
            return false;
        }

        if (pmn->IsNewStartRequired()) {
            LogPrint("indexnode", "CZnodePing::CheckAndUpdate -- indexnode is completely expired, new start is required, indexnode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
    }

    {
        ...
        if ((*mi).second && (*mi).second->nHeight < chainActive.Height() - 24) {
            // LogPrintf("CZnodePing::CheckAndUpdate -- Znode ping is invalid, block hash is too old: indexnode=%s  blockHash=%s\n", vin.prevout.ToStringShort(), blockHash.ToString());
            // nDos = 1;
            return false;
        }
    }

    LogPrint("indexnode", "CZnodePing::CheckAndUpdate -- New ping: indexnode=%s  blockHash=%s  sigTime=%d\n", vin.prevout.ToStringShort(), blockHash.ToString(), sigTime);
    ...
    if (pmn->IsPingedWithin(INDEXNODE_MIN_MNP_SECONDS - 60, sigTime)) {
        LogPrint("indexnode", "CZnodePing::CheckAndUpdate -- Znode ping arrived too early, indexnode=%s\n", vin.prevout.ToStringShort());
    ...
    if (!indexnodeSync.IsZnodeListSynced() && !pmn->IsPingedWithin(INDEXNODE_EXPIRATION_SECONDS / 2)) {
        // let's bump sync timeout
        LogPrint("indexnode", "CZnodePing::CheckAndUpdate -- bumping sync timeout, indexnode=%s\n", vin.prevout.ToStringShort());
        indexnodeSync.AddedZnodeList();
    }

    // let's store this ping as the last one
    LogPrint("indexnode", "CZnodePing::CheckAndUpdate -- Znode ping accepted, indexnode=%s\n", vin.prevout.ToStringShort());
    pmn->lastPing = *this;
    ...
    LogPrint("indexnode", "CZnodePing::CheckAndUpdate -- Znode ping acceepted and relayed, indexnode=%s\n", vin.prevout.ToStringShort());
    Relay();
