...
private:
    ...
    uint32_t property;
    //! amount desired, in IDX
    int64_t IDX_desired_original;
    int64_t min_fee;
...
public:
    ...
    int64_t getIDXDesiredOriginal() const { return IDX_desired_original; }

    CMPOffer()
      : offerBlock(0), offer_amount_original(0), property(0), IDX_desired_original(0), min_fee(0),
    ...
    CMPOffer(int block, int64_t amountOffered, uint32_t propertyId, int64_t amountDesired,
      ...
      : offerBlock(block), offer_amount_original(amountOffered), property(propertyId),
        IDX_desired_original(amountDesired), min_fee(minAcceptFee), blocktimelimit(paymentWindow),
        txid(tx), subaction(0)
    {
    ...
    CMPOffer(const CMPTransaction& tx)
      : offerBlock(tx.block), offer_amount_original(tx.nValue), property(tx.property),
        IDX_desired_original(tx.amount_desired), min_fee(tx.min_fee),
    ...
    void saveOffer(std::ofstream& file, SHA256_CTX* shaCtx, const std::string& address) const
    {
        std::string lineOut = strprintf("%s,%d,%d,%d,%d,%d,%d,%d,%s",
                ...
                property,
                IDX_desired_original,
                (ELYSIUM_PROPERTY_IDX),
                min_fee,
                blocktimelimit,
                txid.ToString()
        );
...
private:
    ...
    int64_t IDX_desired_original;      // copied from the Offer during Accept's creation
...
public:
    ...
    CMPAccept(int64_t amountAccepted, int blockIn, uint8_t paymentWindow, uint32_t propertyId,
      ...
      : accept_amount_remaining(amountAccepted), blocktimelimit(paymentWindow),
        ...
        IDX_desired_original(amountDesired), offer_txid(txid), block(blockIn)
    {
