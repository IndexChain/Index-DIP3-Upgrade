...
int populateRPCTransactionObject(const CTransaction& tx, const uint256& blockHash, UniValue& txobj, std::string filterAddress, bool extendedDetails, std::string extendedDetailsFilter, int blockHeight)
{
    ...
    // DEx IDX payment needs special handling since it's not actually an Elysium message - handle and return
    if (parseRC > 0) {
        if (confirmations <= 0) {
...
void populateRPCTypeTradeOffer(CMPTransaction& elysiumObj, UniValue& txobj)
{
    ...
    int64_t amountDesired = temp_offer.getIDXDesiredOriginal();
