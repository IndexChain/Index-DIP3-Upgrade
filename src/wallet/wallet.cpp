...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nTime, int64_t nSearchInterval, CAmount& nFees, CMutableTransaction& tx, CKey& key, CBlockTemplate *pblocktemplate)
{
    ...
    // Calculate reward
    {
        ...
        bool Dip3Active = nHeight >= Params().GetConsensus().DIP0003EnforcementHeight;
...
{
    {
        ...
        if (posInBlock != -1) {
            if(!(tx.IsCoinBase() || tx.IsCoinStake() || tx.IsSigmaSpend() || tx.IsZerocoinRemint() || tx.IsZerocoinSpend())) {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 IDX."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
