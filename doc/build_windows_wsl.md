 ...
 * [Xming](http://www.straightrunning.com/XmingNotes/) latest version, if you are planning to compile using [unix compiling instruction](https://github.com/indexofficial/index/blob/master/doc/build-unix.md)
...
4. Make shared folder between two locations
    ...
    * From Explorer create a directory named:
            ...
            C:\Users\<username>\index
    * From Ubuntu bash window create a link to it:
  
            ln -s /mnt/c/Users/<username>/index index   
...
5. Clone git repository into newly created directory and go to the right branch
            ...
            git clone https://github.com/indexofficial/index.git
            cd index
...
3. Go to depends folder and build dependencies (you may wish to build only 32-bit or 64-bit version)
        ...
        cd ~/index
    It takes a while. You need to do it only once unless you delete index-depends directory
...
8. After check the directory to run GUI with __index-qt.exe__
        ...
        C:\Users\<username>\index\src\qt
----
## Second method - Ubuntu + Xming
1. Use existing paper [build-unix](https://github.com/indexofficial/index/blob/master/doc/build-unix.md) and
...
4. From Ubuntu bash window start index-qt:

        cd /mnt/c/Users/<username>/index/src/qt
        ./index-qt 
